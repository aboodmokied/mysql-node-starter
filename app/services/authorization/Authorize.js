    ...
    #roleAggregationFunctions(model){
        ...
        // Class Level
            ...
            model.getAvaliableRoles=async function(userId){
                const user=await model.findByPk(userId);
                const roles=await Role.findAll({
                    where:{id:{[Op.notIn]:Application.connection.literal(`(SELECT roleId FROM user_has_roles WHERE userId=${userId} AND guard=${user.guard})`)}},
                })
                return roles;
            }
        ...
        // Instance Level
            ...
            model.prototype.getAvaliableRoles=async function(){
                const roles=await Role.findAll({
                    where:{id:{[Op.notIn]:Application.connection.literal(`(SELECT roleId FROM user_has_roles WHERE userId=${this.id} AND guard=${this.guard})`)}},
                })
                return roles;
            }
    }
